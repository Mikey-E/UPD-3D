...
def process_pts(pts, data_args):
    ...
    # print(f"before pts: {pts.shape}") #@@@new

    #@@@orig
    # if data_args.with_color:
    #@@@new
    if data_args.with_color and pts.shape[1] == 6:

        if pts[:, 3:].max() > 1.0 + 1e-2:
            pts[:, 3:] = pts[:, 3:] / 255.0
    #@@@new
    elif data_args.with_color and pts.shape[1] == 3:
        pts = np.concatenate([pts, np.zeros((pts.shape[0], 3))], axis=1)

    else:
        pts = pts[:, :3]
    pts = torch.from_numpy(pts).float()

    # print(f"after pts: {pts.shape}") #@@@new
    return pts
