...
#SBATCH --output=./slurm_logs/gplm_train_stage2_%j.out
#SBATCH --error=./slurm_logs/gplm_train_stage2_%j.out
#SBATCH --gres=gpu:1
#SBATCH --mem=64G
...
# Usage: sbatch slurm_gplm_train_stage2.sh <experiment_name>
#    or: EXP_NAME=<experiment_name> sbatch ...
if [[ -n "${1:-}" ]]; then
    export EXP_NAME="$1"
fi
: "${EXP_NAME:?Pass experiment name as arg1 or set EXP_NAME}"
echo "EXP_NAME=${EXP_NAME}  (checkpoints → ./checkpoints/stage_2_${EXP_NAME})"
